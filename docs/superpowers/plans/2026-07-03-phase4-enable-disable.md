...
T5|✅ done|StatusLineVM TaskCompleted + facade setDeviceEnabled/canDisable|committed ba28323; 106/106 green (+7); tidy+format clean; delta: TaskCompletedEvent constructions need designated initializers (modernize-use-designated-initializers gate)
T6|✅ done|sdbus gating + dbus_contract + ManagerAdaptor + PolkitAuthority + devmgrd|committed 80b593c; 109/109 green (+3); tidy+format clean; session-bus busctl smoke OK (authorized 1→0); deltas: sdbus find_package+option moved BEFORE add_subdirectory(daemon) (plan said after Qt6 block, but daemon subdir is processed earlier — option/SDBusCpp target otherwise invisible to it); main.cpp parse() → flag→member map + std::span (plan's else-if chain ≈ complexity 16 > 15 gate; pro-bounds-pointer-arithmetic on argv), std::cerr not fprintf (pro-type-vararg); polkit timeout 110 → kPolkitCallTimeout const (magic-number gate); NOLINT swappable-params on checkAuthorized def (CallerId aliases std::string, T4 precedent); awaiting user commit
T7|✅ done|DbusPrivilegedChannel + IPC round-trip suite (tests/ipc)|committed 61d4f6d; 110/110 green (+1 ctest entry = 6 gtest round-trips on private session bus, 0.3 s); tidy 0 errors on gated dbus_privileged_channel.cpp; format clean as written; sole delta: Step-2 expectation — configure does NOT fail (same as T4 correction), failing state = compile error on missing dbus_privileged_channel.hpp; awaiting user commit
T8|✅ done|TUI wiring ('e' toggle + confirm + auto-refresh)|110/110 green (no new tests, as planned); format clean after -i pass; deltas: PendingToggle armed with designated initializers (tidy style); Step-3 correction — `devmgr-tui </dev/null` does NOT exit on EOF (pre-existing, verified against HEAD binary too; plan expectation stale) — replaced by `printf q | devmgr-tui` full-loop sanity (exit 0) + SIGTERM teardown observed clean; verified refresh() publishes no TaskCompletedEvent so the ok-filtered auto-refresh subscription cannot self-loop; awaiting user commit
T9|✅ done|GUI wiring (toggle action + confirm + auto-refresh)|114/114 green (+4 window tests; existing 5 fixture updated); selftest rows=128 exit 0; format clean after -i pass; gui tidy 0 (sed `-mno-direct-extern-access` workaround, 18 NOLINT = phase-3 Qt-idiom suppressions); deltas: added `<optional>`/`<utility>` includes for std::nullopt/std::move (plan snippets omitted them); MainWindow ctor gained `facade` as first param + toggleAction_/onSetEnabled_/confirm_ members; awaiting user commit
T10|✅ done|Dockerfile sdbus build + CI guards + README + VM script|Dockerfile: +`libsystemd-dev dbus` apt + pinned sdbus-c++ v2.3.1 source layer after vcpkg (find_package auto-flips DEVMGR_WITH_SDBUS=ON — no explicit flag); ci.yml: format/toolkit-purity/tidy roots +`daemon`, new sdbus-purity guard step (verified both polarities locally: sdbus includers = daemon/src/* + the one channel leaf, guard CLEAN); test/vm/phase4-smoke.sh created +x (bash -n clean); README Phase 4 section (was empty file); no C++ touched → format N/A; **container run 115/115 green** (host 114 + `devmgr_integration` umockdev mapper test; `devmgr_ipc` + `devmgr_integration` both pass; 1 root-only skip `UnwritableAttrIsIo`); env note: `podman-compose run` fails on rootless netavark (`ip_tables` insmod denied) → ran built image directly `podman run --rm --network none localhost/test_unit:latest ctest …` (suite needs no network; IPC uses a private session bus); awaiting user commit
