...
T7|✅ done|DbusPrivilegedChannel + IPC round-trip suite (tests/ipc)|110/110 green (+1 ctest entry = 6 gtest round-trips on private session bus, 0.3 s); tidy 0 errors on gated dbus_privileged_channel.cpp; format clean as written; sole delta: Step-2 expectation — configure does NOT fail (same as T4 correction), failing state = compile error on missing dbus_privileged_channel.hpp; awaiting user commit
