...
// Updates). The Devices page keeps the Phase 3 spec shape (filter box +
// grouped device list left, key/value detail pane right); the Modules page
// mirrors the T11 TUI screen (Secure Boot banner + filter + fixed-column
...
// root keeps owning future custody; Updates commands call the facade
// directly and this window keeps its own future custody (pending_, drained in
// the destructor) — the drain-before-teardown contract in
...
class MainWindow final : public QMainWindow {
   ...
   public:
    ...
    MainWindow(app::ApplicationFacade& facade, app::DeviceListVM& listVm,
               ...
               app::ModulesVM& modulesVm, app::UpdatesVM& updatesVm, QtUiDispatcher& dispatcher,
               runtime::EventBus& bus, Actions actions, QWidget* parent = nullptr);
    ~MainWindow() override;
