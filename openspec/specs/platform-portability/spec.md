...
### Requirement: A hotplug removal is correlated by platform-native identity
The shared device model SHALL identify the device a removal event refers to by the platform-native identity the event carries, not solely by the enumeration-derived device key. An operating system that omits from a removal notification the properties the enumeration-derived key is built from SHALL still cause the device to leave the model and a single removal to be published.

#### Scenario: Removal with a degraded event identity still removes the device
- **WHEN** a device is in the model from an arrival carrying full identity, and a removal event for it arrives carrying the same platform-native identity but not the properties the enumeration-derived key is built from
- **THEN** the device is removed from the shared model and one removal is published carrying the model's own key for that device

#### Scenario: A removal event that matches nothing changes nothing
- **WHEN** a removal event carries a platform-native identity that no device in the model holds, or carries none
- **THEN** the model is unchanged and no removal is published
