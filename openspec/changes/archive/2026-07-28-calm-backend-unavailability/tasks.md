...
- [x] 11.4 **Owner runs:** confirm the raw diagnostic appears in the log exactly once per transition, not once per poll, over a few minutes with the backend down. **Once-per-poll half PASSES (2026-07-28):** ten Snapshots-tab entries, each a real `snapshotList()` against an unreachable daemon, produced exactly one warn line — captured from `devmgr-gui`'s stdout, since spdlog's default logger writes to stdout and the TUI is drawing on it. **Second-outage half PASSES (2026-07-28, 12:08–12:09):** owner installed `devmgrd` as an OpenRC service (this host has no systemd) and drove down→up→down with `rc-service` against one GUI instance; two `devmgrd` warn lines 72s apart. The log proves the transition on its own — `observe()` clears `logged_` only on an error-free call and otherwise re-logs only on a kind change, and both lines read `(unreachable)`, so the second could only follow a successful call in between. Detail in `manual-test-note.md` §11.4.
